...
main(int argc, char** argv)
{
  ...
  if (appMode == AUTO) {
    ...
    std::thread getURLThread(&Controller::GetPlaybackURLFromUser, controller,
                             std::move(prms));
    ...
    if (status == std::future_status::ready) // result is ready
    {
      ...
      std::cout << " Failed to get playback URL!!! Exiting application \n";
      exit(EXIT_SUCCESS);
    }
     ...
     */
    std::thread PlaybackCntrThread(&Controller::ControllPlayback, controller);
