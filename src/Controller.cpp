...
Controller::ControllPlayback()
{
  while (true) {
    ...
    if (ops == PLAY) {
      ...
      std::cout << "GSTREAMER ERROR: Failed to playback the stream. Looks like "
...
void
Controller::ControllSwapPlayback(const PlayOps& ops)
{
  std::cout << "User selected operation = " << ops << std::endl;
  if (ops == PLAY) {
    if (!_url.empty()) {
...
void
Controller::GetPlaybackURLFromUser(std::promise<std::string>&& prms)
{
...
gboolean
Controller::UserInCallback(GIOChannel* source, GIOCondition condition,
                           gpointer data)
{
  gchar* str = NULL;
  gsize str_len = 0;
  ...
  if (NULL != str) {
    if (strncmp(str, "play", 4) == 0) {
      ...
      std::cout << "Invalid user input!!! Use from (play/pause/stop/quit) \n";
    }
  }
  return true;
}
...
void
Controller::SetPlaybackUrl(std::string& url)
{
  _url = url;
}
